export default function ExperienceTimeline({items}){
  return <section id="experience" className="section experience-section"><div className="container">
    ...
    <ol className="timeline">{items.map((item,index)=><li key={item.role+item.company}><span className="timeline-index">0{index+1}</span><article><div className="role-header"><div><p className="period">{item.period}</p><h3>{item.role}</h3><p className="company">{item.company} <span>·</span> {item.location}</p></div><span className="role-type">{index===0?'Current role':'Foundation'}</span></div><ul>{item.highlights.map(highlight=><li key={highlight}>{highlight}</li>)}</ul></article></li>)}</ol>
  </div></section>;
}
